  ...
  <div class="sm:p-10 my-auto">
    <section class="mx-auto max-w-screen-xl md:rounded-md md:border md:shadow-lg">
      <div class="grid grid-cols-4 text-gray-800 lg:grid-cols-3">
        <div
          class="col-span-4 bg-gray-50 px-8 py-10 text-gray-800 md:col-span-2 md:border-r md:px-10 md:py-12 lg:col-span-1">
          ...
          <ul>
            ...
            <li class="my-6 flex items-center text-left">
              <svg class="shrink-0 mr-6 text-2xl text-gray-500" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 1024 1024">
                <path fill="currentColor"
          ...
          <form @submit.prevent="submitForm">
            <div class="md:col-gap-4 mb-5 grid md:grid-cols-2">
              <input v-model="formData.nama"
                ...
                type="text" placeholder="nama" nama="nama" />
              <input v-model="formData.email"
                class="col-span-1 w-full border-b py-3 text-sm outline-none focus:border-b-2 focus:border-black"
                type="email" placeholder="Email" nama="email" />
            </div>
            <textarea v-model="formData.pesan"
              class="mb-10 w-full resize-y whitespace-pre-wrap border-b py-3 text-sm outline-none focus:border-b-2 focus:border-black"
              id="" rows="6" placeholder="pesan" nama="pesan"></textarea>
            <button type="submit"
...
export default {
  ...
  methods: {
    submitForm() {
      // Lakukan sesuatu dengan data formulir yang sudah diisi
      console.log(this.formData);
      // Panggil action untuk mengirim email dengan data formulir
      this.$store.dispatch('kontak/sendEmail', this.formData)
        .then(() => {
          // Handle jika pengiriman email berhasil
          console.log('Email has been sent successfully!');
          // Reset formulir setelah pengiriman email berhasil
          this.resetForm();
        })
        .catch(error => {
          // Handle jika terjadi error saat pengiriman email
